...
struct FloatingSplitView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                VStack(spacing: 0) {
                    // TODO: - [브리] 디자인 확정 시 색 변경 필요
                    if !isVertical && mainPDFViewModel.isPaperViewFirst {
                ...
                if isVertical {
                    HStack(spacing: 0) {
                        Spacer()
                            .frame(width: mainPDFViewModel.isPaperViewFirst ? 0 : nil)
                        ...
                        Spacer()
                            .frame(width: mainPDFViewModel.isPaperViewFirst ? nil : 0)
                    }
                }
            }
        }
